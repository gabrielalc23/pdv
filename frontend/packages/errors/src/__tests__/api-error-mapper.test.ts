...
describe("mapApiError", () => {
  ...
  it("maps 400 to ValidationError", () => {
    const axiosErr = createAxiosError(400, { error: { code: "invalid", message: "Bad request", field: "email" } })
    expect(() => mapApiError(axiosErr)).toThrow(ValidationError)
    try { mapApiError(axiosErr) } catch (e: any) {
      expect(e.field).toBe("email")
    }
  })

  it("maps 422 to ValidationError", () => {
    const axiosErr = createAxiosError(422, { error: { code: "validation", message: "Invalid data", field: "name" } })
    expect(() => mapApiError(axiosErr)).toThrow(ValidationError)
    try { mapApiError(axiosErr) } catch (e: any) {
  ...
  it("maps 404 to NotFoundError", () => {
    const axiosErr = createAxiosError(404, { error: { code: "not_found", message: "Product not found" } })
  ...
  it("maps unknown status to generic ApiError", () => {
    const axiosErr = createAxiosError(500, { error: { code: "server_error", message: "Internal error" } })
