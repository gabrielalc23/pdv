...
describe("useActivateProductMutation", () => {
  it("activates a product", async () => {
    ...
    testServer.use(
      createMockHandler(
        "post",
        `/products/${mockProduct.id}/activate`,
        200,
        activatedProduct,
      ),
    );

    const { result } = renderHook(() => useActivateProductMutation(), {
      wrapper: TestWrapper,
    });
...
describe("useDeactivateProductMutation", () => {
  it("deactivates a product", async () => {
    ...
    testServer.use(
      createMockHandler(
        "post",
        `/products/${mockProduct.id}/deactivate`,
        200,
        deactivatedProduct,
      ),
    );

    const { result } = renderHook(() => useDeactivateProductMutation(), {
      wrapper: TestWrapper,
    });
