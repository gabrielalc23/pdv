...
describe("useAddSaleItemMutation", () => {
  it("adds an item to a sale", async () => {
    ...
    testServer.use(
      createMockHandler(
        "post",
        `/sales/${mockSale.id}/items`,
        200,
        saleWithItem,
      ),
    );

    const { result } = renderHook(() => useAddSaleItemMutation(), {
      wrapper: TestWrapper,
    });
...
describe("useUpdateSaleItemMutation", () => {
  it("updates a sale item", async () => {
    ...
    testServer.use(
      createMockHandler(
        "put",
        `/sales/${mockSale.id}/items/${mockSale.items[0].id}`,
        200,
        saleWithUpdatedItem,
      ),
    );

    const { result } = renderHook(() => useUpdateSaleItemMutation(), {
      wrapper: TestWrapper,
    });
...
describe("useRemoveSaleItemMutation", () => {
  it("removes an item from a sale", async () => {
    ...
    const { result } = renderHook(() => useRemoveSaleItemMutation(), {
      wrapper: TestWrapper,
    });
...
describe("useCancelSaleMutation", () => {
  it("cancels a sale", async () => {
    ...
    testServer.use(
      createMockHandler(
        "post",
        `/sales/${mockSale.id}/cancel`,
        200,
        cancelledSale,
      ),
    );

    const { result } = renderHook(() => useCancelSaleMutation(), {
      wrapper: TestWrapper,
    });
